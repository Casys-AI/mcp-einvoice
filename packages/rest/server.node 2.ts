/**
 * E-Invoice REST Server — Node.js entry point
 *
 * Uses @hono/node-server instead of Deno.serve.
 * All routes are in src/app.ts which is runtime-agnostic.
 *
 * @module einvoice-rest/server.node
 */

import process from "node:process";
import { serve } from "@hono/node-server";
import { createAdapter } from "@casys/einvoice-core";
import { createApp } from "./src/app.ts";

const DEFAULT_PORT = 3016;

function main() {
  const args = process.argv.slice(2);

  const adapterArg = args.find((a) => a.startsWith("--adapter="));
  const adapterName = adapterArg
    ? adapterArg.split("=")[1]
    : process.env["EINVOICE_ADAPTER"] || "iopole";

  const portArg = args.find((a) => a.startsWith("--port="));
  const port = portArg
    ? parseInt(portArg.split("=")[1], 10)
    : parseInt(process.env["PORT"] || String(DEFAULT_PORT), 10);

  const noAuth = args.includes("--no-auth");
  const apiKey = noAuth ? null : (process.env["EINVOICE_REST_API_KEY"] ?? null);
  if (!apiKey && !noAuth) {
    console.error(
      "[einvoice-rest] WARNING: No EINVOICE_REST_API_KEY set. Use --no-auth to disable auth explicitly.",
    );
  }

  const adapter = createAdapter(adapterName);
  const app = createApp(adapter, apiKey);

  console.error(
    `[einvoice-rest] Starting — adapter=${adapterName}, port=${port}, auth=${
      apiKey ? "enabled" : "disabled"
    }`,
  );

  serve({ fetch: app.fetch, port });
}

main();
