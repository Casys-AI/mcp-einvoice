...
const PLATFORM_STATE: Record<string, StatusScheme> = {
  ...
  conversion_failed: {
    color: colors.error,
    bg: colors.errorDim,
    label: "Conversion échouée",
    labelEn: "Conversion failed",
    icon: "error",
  },
};
...
export function getStatus(code: string): StatusScheme {
  ...
  // Legacy alias
  if (ALIASES[key] && STATUS_REGISTRY[ALIASES[key]]) {
    return localize(STATUS_REGISTRY[ALIASES[key]]);
  }
